...
Contract version: **`pass1-output-contract-v0.1`** (frozen-literal; rev-locked under PR#18e). Any future change requires a new contract version stamp and its own PR.
...
Pass1Candidate {
  ...
  pass1_contract_version:       'pass1-output-contract-v0.1'
  ...
  risk_evidence_status:         'unavailable' | 'warning' | 'usable_later'
                                                    // 'warning' while PR#18d §8 carry-forward is open
                                                    // 'unavailable' if observer reported failed query
                                                    // 'usable_later' is reserved for after the warning is resolved
...
- **`risk_evidence_status`**: while the PR#18d §8 carry-forward warning is open, this field is `'warning'` (or `'unavailable'` for candidates whose evidence_refs do not reference any risk source). It MUST NOT be `'usable_later'` until the warning is closed via a separately-gated PR (§7).
- **`customer_claim_allowed`** and **`lane_output_allowed`**: ALWAYS `false` in this contract. Future PRs that want to flip either flag must amend the contract under a new version stamp (`pass1-output-contract-v0.2`) with their own PR + Helen GO + Codex review.
...
| `blocked_by_risk_warning` | The candidate's `evidence_refs` reference risk-observation evidence AND PR#18d §8's `risk_observations_v0_1` warning is still open. Pass 1 categorically refuses to interpret based on potentially-unavailable risk evidence. `risk_evidence_status = 'warning'`. `pass_to_trust_planning = false`. |

### 5.1 What is explicitly NOT in the enum

No `high_intent`, no `ready_to_buy`, no `human_visitor`, no `bot_visitor`, no `ai_agent_detected`, no `invalid_traffic`, no `lead_quality`, no `conversion_likely`, no `revenue_eligible`, no customer-facing language of any kind. These categorical absences are part of the v0.1 contract and removable only via a new contract version.
...
- The `Pass1Candidate.risk_evidence_status` field will **not** report `'usable_later'` until a PR that closes this warning is merged and explicitly cited by an amendment to PR#18e (`pass1-output-contract-v0.2`).
- The `blocked_by_risk_warning` interpretation enum will NOT be removed from §5 until the warning is closed.
...
- **Clear Pass 1 input / output contract.** §2 enumerates the allowed inputs (the observer's pass-forward shape); §3 enumerates forbidden inputs; §4 locks the `Pass1Candidate` shape under contract version `pass1-output-contract-v0.1`; §5 locks the 8-value `Pass1Interpretation` enum; §6 locks the forbidden-claim list.
...
End of PR#18e. **Verdict: PLANNING ONLY — Pass 1 output contract planning under contract version `pass1-output-contract-v0.1`. The Pass 1 candidate shape is locked (§4), the allowed-interpretation enum is closed at 8 categorical values (§5), the forbidden-claim list is enumerated (§6), the PR#18d `risk_observations_v0_1` warning is carried forward (§7) and bound to the `risk_evidence_status` field + `blocked_by_risk_warning` interpretation, the relationships to Trust (§8) / Pass 2 (§9) / Lane A/B (§10) are documented as separately-gated downstream steps, and the non-goals list (§11) preserves all boundaries from PR#18a / PR#18b. No Pass 1 runtime, no Trust runtime, no Pass 2 runtime, no customer-facing output, no Lane A/B writer, no AMS runtime bridge, no Gate 4 work, no production `endpointUrl` re-flip, no production traffic, no Track A, no Playwright, no knobs / dashboard, no website ThinSDK production activation, no production artefact / config mode flip, no AMS repo modification, no code / scripts / tests / package / migrations / schema.sql / env / systemd / Nginx / AMS source / website artefact / production config changes are approved or introduced by PR#18e. PR#18f (Trust planning), PR#18g (Lane A/B governance planning), and any Gate 4 PR thereafter remain separately gated by their own explicit Helen GO.**
