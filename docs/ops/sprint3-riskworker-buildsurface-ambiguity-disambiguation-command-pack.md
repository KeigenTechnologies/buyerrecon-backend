# Sprint 3 — Risk-Worker `build_surface` Ambiguity Disambiguation Diagnostic Command-Pack (Review-Only)

> **Docs-only, review-only.** Defines the command-pack boundary and review checklist for a **future
> controlled** disambiguation diagnostic between the two active `build_surface` safe signals recorded
> by the merged evidence (PR #384) and planned in PR #385:
> `tsx_ts_node_runtime_surface_signal = true` and `ci_build_parity_surface_signal = true`
> (current sublabel `unknown_build_surface`).
>
...
> fix, no retry, no root-cause inference. It is a reviewable specification only. Execution requires
> this command-pack reviewed and merged, then a **separate exact operator GO**.
>
...
> `unknown_build_surface`; active signals `tsx_ts_node_runtime_surface_signal = true` and
> `ci_build_parity_surface_signal = true`; `runtime_cause_inference = false`. Neither `build_surface`
> nor `unknown_build_surface` unblocks Gate D/E.

---

## STATUS: `SPRINT3_RISKWORKER_BUILDSURFACE_AMBIGUITY_DISAMBIGUATION_COMMAND_PACK_REVIEW_ONLY`
...
that distinguishes whether the active dual-signal ambiguity is best explained by exactly one of the
following **safe-pattern-family labels**, **without asserting a root cause**:

- `tsx_runtime_reliance_surface`
- `ci_build_parity_surface`
- `structurally_expected_dual_signal`
- `still_unknown_build_surface`

The diagnostic classifies into exactly one of the four labels and stops.

The objective explicitly is **not**:

- to run the risk-worker, a classifier, risk-evidence, or a record-only rerun;
- to read or print raw logs, raw runtime output, exact errors, stack traces, dependency-file contents,
  env values, DSNs, hosts, private paths, SQL results, customer data, or base64 blobs;
...
stop labels. A `still_unknown_build_surface` (or `structurally_expected_dual_signal`) classification is
an expected, acceptable outcome and authorizes no escalation.

---

## 2. Exact future GO phrase

- Exact GO phrase (placeholder):
...
- [ ] Parent planning PR #385 merged; sublabel evidence PR #384 merged.
...
- [ ] Head pinned: execution runs against a specific, recorded head SHA on
      `sprint2-architecture-contracts-d4cc2bf` at or after `93e4e627b2c21b6cdd34dbc2aa29331efb5b6e15`.
...
- [ ] Static/read-only presence-and-shape checks that map the four candidate labels (Section 1) to
      boolean/count signals — e.g. counts of risk scripts invoking a runtime loader (`tsx`) vs presence
      of a discrete build step, counts of CI/build parity assertions, and a joint-structure check for
      whether both signals are structurally co-expected — **presence/shape/count only, never contents**.
- [ ] Classify into exactly one label (Section 1) and stop at classification.
...
| --- | --- |
| Safe labels (`PREFLIGHT_OK`, `DISAMBIGUATE_DONE`, one label from Section 1) | Yes — required |
| Booleans (e.g. `build_step_present=true/false`, `signals_co_expected=true/false`) | Yes |
| Aggregate counts (e.g. `runtime_loader_invocation_count=<n>`, `parity_assertion_count=<n>`) | Yes |
...
**Explicit output ban:** the future diagnostic must **never** print raw logs, exact errors, stack
traces, dependency-file contents, env values, private paths, DSNs, hosts, SQL results, customer data,
or base64 blobs. Any risk of such leakage is a fail-closed condition (Section 13).

---

## 11. No-op / rollback boundary
...
- Expected evidence artifact: a safe-labelled diagnostic note (labels / booleans / aggregate counts +
  interpretation only).
- Expected safe labels: `PREFLIGHT_OK`, `DISAMBIGUATE_DONE`, one label from Section 1, plus supporting
  booleans/counts, and `stop_label` if blocked.
...
- **Success:** preflight passed, one read-only/static discrimination pass completed, exactly one label
  (Section 1) emitted, no boundary crossed, sealed state and `runtime_cause_inference: false` preserved.
- **Blocked:** any fail-closed criterion tripped; `stop_label` records the reason token.
- **Still ambiguous:** if signals remain non-distinguishing, the label is `still_unknown_build_surface`
  (or `structurally_expected_dual_signal` if both are co-expected). Acceptable; authorizes no escalation.
...
## 15. Negative-action ledger (this command-pack PR)
...
- [ ] Objective is strictly read-only/static and matches Section 1; the four labels are safe-pattern-family labels.
...
- [ ] Safe output contract and explicit output ban (Section 10) permit labels/booleans/counts/stop-labels only.
- [ ] No-op/rollback boundary (Section 11) confirms read-only/static.
- [ ] Fail-closed criteria (Section 13) trip on branch/tree/GO/leakage anomalies.
- [ ] Evidence and interpretation rules (Sections 12, 14) require a separate docs-only evidence PR.
- [ ] Sealed state preserved: `runtime_dependency_or_build_failure`; `blocked / none_not_classified`;
      `build_surface` / `unknown_build_surface` safe labels; `runtime_cause_inference=false`.
- [ ] Gate D / Gate E boundary (Section 18) honored.
...
```text
STATUS: SPRINT3_RISKWORKER_BUILDSURFACE_AMBIGUITY_DISAMBIGUATION_COMMAND_PACK_REVIEW_ONLY
OPERATION: Risk-worker build_surface ambiguity disambiguation read-only diagnostic
BASE: sprint2-architecture-contracts-d4cc2bf @ <pinned head SHA >= 93e4e62>
...
STEP 1 static disambiguation pass: <DISAMBIGUATE_DONE | RESULT_BLOCKED>
  runtime_loader_invocation_count .. <n>
  parity_assertion_count ........... <n>
  build_step_present ............... <true|false>
  signals_co_expected .............. <true|false>

RESULT_LABEL: <tsx_runtime_reliance_surface | ci_build_parity_surface | structurally_expected_dual_signal | still_unknown_build_surface>
STOP_LABEL: <none | reason-token>
SEALED STATE: build_surface / unknown_build_surface unchanged; runtime_cause_inference=false
SAFE LABELS ONLY: yes
RAW LOGS / OUTPUT / ERRORS / STACKS / SQL / DEP-CONTENTS / ENV / PRIVATE PATHS / CUSTOMER DATA / SECRETS: none
...
## 18. Gate D / Gate E boundary statement
...
- No disambiguation label outcome (`tsx_runtime_reliance_surface`, `ci_build_parity_surface`,
  `structurally_expected_dual_signal`, or `still_unknown_build_surface`) unblocks any Gate or implies
  scoring/customer-output readiness.

---

_End of command-pack. Docs-only, review-only. Authorizes no fix, execution, retry, Gate D/E, or customer-output action. Sealed state preserved._
