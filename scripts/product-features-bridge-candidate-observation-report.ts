#!/usr/bin/env tsx
/**
 * Sprint 2 PR#14c — ProductFeatures Bridge Candidate Observer CLI.
 *
 * Read-only internal diagnostic. Reuses PR#13b's preview pipeline
 * + PR#14b's pure bridge mapper to emit
 * `product_features_namespace_candidate` samples from real
 * staging POI / POI Sequence evidence. Markdown report to stdout.
 *
 * STRICTLY READ-ONLY. No INSERT / UPDATE / DELETE. No DDL. No psql.
 * No durable bridge table. No customer output. No AMS Product Layer
 * runtime execution. No `ProductDecision`. No Render production
 ...
 *
 * Exit codes:
 *   0 — report rendered, source readiness OK, candidate generation OK
 *   2 — env / connection / SQL error / fail_closed source readiness /
 *       candidate generation failure (all inputs rejected)
 */
...
function fail(code: number, msg: string): never {
  process.stderr.write(`PR#14c product-features-bridge-candidate observer — ${msg}\n`);
  process.exit(code);
}
...
function parseEnv(env: NodeJS.ProcessEnv = process.env, now: Date = new Date()): ParsedEnv {
  ...
  const category_template:       CategoryTemplate       = parseEnumOrFail(env.OBS_CATEGORY_TEMPLATE,       CATEGORY_TEMPLATES_ALLOWED,       DEFAULT_CATEGORY_TEMPLATE,       'OBS_CATEGORY_TEMPLATE');
  const primary_conversion_goal: PrimaryConversionGoal  = parseEnumOrFail(env.OBS_PRIMARY_CONVERSION_GOAL, PRIMARY_CONVERSION_GOALS_ALLOWED, DEFAULT_PRIMARY_CONVERSION_GOAL, 'OBS_PRIMARY_CONVERSION_GOAL');
  const sales_motion:            SalesMotion            = parseEnumOrFail(env.OBS_SALES_MOTION,            SALES_MOTIONS_ALLOWED,            DEFAULT_SALES_MOTION,            'OBS_SALES_MOTION');
...
async function main(): Promise<void> {
  ...
  let report;
  try {
    report = await runBridgeCandidateObserver({
      client:        pool,
  ...
  process.stdout.write(renderBridgeCandidateObserverMarkdown(report));
  process.stdout.write('\n');

  const decision = report.exit_decision;
  if (decision.exit_code !== 0) {
    if (decision.stderr_message !== null) {
      process.stderr.write(`PR#14c product-features-bridge-candidate observer — ${decision.stderr_message}\n`);
    }
...
main().catch((err) => {
  process.stderr.write(`PR#14c product-features-bridge-candidate observer — fatal: ${(err as Error).message}\n`);
  process.exit(2);
});
