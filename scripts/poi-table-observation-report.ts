#!/usr/bin/env tsx
/**
 * Sprint 2 PR#11d — POI Observations Table Observer CLI.
 *
 * Read-only internal engineering diagnostic. Reads
 * `poi_observations_v0_1` + `information_schema.columns` only.
 * Verifies the 9 row-level invariants from migration 014 / the
 * verification SQL, plus a schema-level forbidden-column sweep.
 * Emits a JSON report to stdout. Writes nothing.
 *
 * STRICTLY READ-ONLY. No INSERT / UPDATE / DELETE. No DDL. No psql.
 * No Lane A/B reads. No raw upstream reads. No customer output. No
 ...
 *   OBS_LIMIT                      default 10000 (per anomaly check)
 ...
 *   OBS_POI_INPUT_VERSION          default POI_CORE_INPUT_VERSION
 *   OBS_POI_OBSERVATION_VERSION    default POI_OBSERVATION_VERSION_DEFAULT
 *
 * Exit codes:
...
function fail(code: number, msg: string): never {
  process.stderr.write(`PR#11d poi-table observer — ${msg}\n`);
  process.exit(code);
}
...
function parseEnv(env: NodeJS.ProcessEnv = process.env, now: Date = new Date()): ParsedEnv {
  ...
  const poi_input_version_expected = typeof env.OBS_POI_INPUT_VERSION === 'string' && env.OBS_POI_INPUT_VERSION.length > 0
    ? env.OBS_POI_INPUT_VERSION : POI_CORE_INPUT_VERSION;
  const poi_observation_version_expected = typeof env.OBS_POI_OBSERVATION_VERSION === 'string' && env.OBS_POI_OBSERVATION_VERSION.length > 0
    ? env.OBS_POI_OBSERVATION_VERSION : POI_OBSERVATION_VERSION_EXPECTED;
...
async function main(): Promise<void> {
  ...
  let report;
  try {
    report = await runPoiTableObserver({
      client:        pool,
  ...
  if (parsed.require_rows && report.rows_in_table === 0) {
    process.stderr.write('PR#11d poi-table observer — OBS_REQUIRE_ROWS=true and rows_in_table === 0\n');
    process.exit(1);
  }
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(`PR#11d poi-table observer — fatal: ${(err as Error).message}\n`);
  process.exit(2);
});
