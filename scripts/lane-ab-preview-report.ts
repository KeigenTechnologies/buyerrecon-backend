#!/usr/bin/env tsx
/**
 * Sprint 2 PR#16b — Lane A / Lane B Evidence Review preview observer CLI.
 *
 * Read-only internal diagnostic. Chains the PR#15a Evidence Review
 * snapshot into the PR#16b preview projection and prints the
 * customer-shaped Lane A / Lane B preview markdown to stdout.
 *
 * STRICTLY READ-ONLY. No INSERT / UPDATE / DELETE. No DDL. No
 * durable Lane-A / Lane-B writer. No AMS Product Layer execution.
 * No `ProductDecision`. No `RequestedAction`. No Render production
 * deploy.
 *
 * Env (required):
...
function fail(code: number, msg: string): never {
  process.stderr.write(`PR#16b lane-ab-preview observer — ${msg}\n`);
  process.exit(code);
}
...
async function main(): Promise<void> {
  ...
  let snapshot;
  try {
    snapshot = await runEvidenceReviewSnapshot({
      client:        pool,
      options:       {
  ...
  await pool.end();
  const preview = buildLaneABPreview(snapshot);
  process.stdout.write(renderLaneABPreviewMarkdown(preview));
  process.stdout.write('\n');
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`PR#16b lane-ab-preview observer — fatal: ${(err as Error).message}\n`);
  process.exit(2);
});
