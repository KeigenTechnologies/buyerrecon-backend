#!/usr/bin/env tsx
/**
 * Sprint 2 PR#12b — POI Sequence Observer CLI.
 *
 * Read-only internal engineering diagnostic. Reads
 * `poi_observations_v0_1` + `information_schema.tables` only. Groups
 * POI rows by (workspace_id, site_id, session_id) and emits a JSON
 * report of in-session POI Sequence facts. Writes nothing.
 *
 * STRICTLY READ-ONLY. No INSERT / UPDATE / DELETE. No DDL. No psql.
 * No Lane A/B reads. No raw upstream reads. No customer output.
 ...
 *   OBS_LIMIT                      default 50000 (row cap)
 ...
 *   OBS_POI_OBSERVATION_VERSION    default POI_OBSERVATION_VERSION_EXPECTED
 *
 * Exit codes:
...
function fail(code: number, msg: string): never {
  process.stderr.write(`PR#12b poi-sequence observer — ${msg}\n`);
  process.exit(code);
}
...
function parseEnv(env: NodeJS.ProcessEnv = process.env, now: Date = new Date()): ParsedEnv {
  ...
  const poi_input_version_expected =
    typeof env.OBS_POI_INPUT_VERSION === 'string' && env.OBS_POI_INPUT_VERSION.length > 0
...
async function main(): Promise<void> {
  ...
  let report;
  try {
    report = await runPoiSequenceObserver({
      client:        pool,
  ...
  if (parsed.require_rows && report.rows_scanned === 0) {
    process.stderr.write('PR#12b poi-sequence observer — OBS_REQUIRE_ROWS=true and rows_scanned === 0\n');
    process.exit(1);
  }
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(`PR#12b poi-sequence observer — fatal: ${(err as Error).message}\n`);
  process.exit(2);
});
